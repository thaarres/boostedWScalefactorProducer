...
parser = OptionParser()
parser.add_option('-b', action='store_false', dest='noX', default=True, help='no X11 windows')
...
def GetWtagScalefactors(workspace,fitter):
    ...
    #------------- Calculate W-tagging scalefactors -------------
...
class initialiseFits:
    ...
    def __init__(self, in_channel, in_sample, in_mj_min=40, in_mj_max=130, input_workspace=None):
      ...
      print "Using Tau21 HP cut =   " ,options.tau2tau1cutHP 
      ...
      self.mj_shape["signal_mc_fail"]       	= "GausErfExp_ttbar_failtau2tau1cut"
      ...
      if (options.useDDT): 
        ...
        # OLD METHOD, EG ADDING TWO ADDITIONAL FIT FUNCTIONS
	...
	# self.mj_shape["signal_data_fail"]     = "GausChebychev_ttbar_failtau2tau1cut"
      ...
      #Set lumi  
      self.Lumi=12900
      if options.use76X: self.Lumi=2300.
	...
	#FIXME! Add method to save workspace so that it does not have to be recreated each time
    ...
    def get_sim_fit_components(self):
      ...
      self.workspace4fit_.var("tt_scalefactor").Print()
        
    # CURRENTLY NOT IN USE! Useful for debugging
    ...
    def get_mj_dataset(self,in_file_name, label, jet_mass="Whadr_pruned"): 
      ...
      rrv_number_before                       = RooRealVar("rrv_number_ttbar"+label+"_beforetau2tau1cut_em_mj","rrv_number_ttbar"+label+"_beforetau2tau1cut_em_mj",0.,10000000.) #For simple counting SF

      # Mj dataset in HP category
      ...
      rrv_number_pass                         = RooRealVar("rrv_number_ttbar"+label+"_passtau2tau1cut_em_mj","rrv_number_ttbar"+label+"_passtau2tau1cut_em_mj",0.,10000000.) #For simple counting SF
 
      # Mj dataset failing HP cut
      ...
      rrv_number_fail                        = RooRealVar("rrv_number_ttbar"+label+"_failtau2tau1cut_em_mj","rrv_number_ttbar"+label+"_failtau2tau1cut_em_mj",0.,10000000.) #For simple counting SF

      ### Mj dataset extreme fail > LPmax
      ...
      rrv_number_extremefail                 = RooRealVar("rrv_number_ttbar"+label+"_extremefailtau2tau1cut_em_mj","rrv_number_ttbar"+label+"_extremefailtau2tau1cut_em_mj",0.,10000000.) #For simple counting SF
	  ...
	  #Check if W-jet is real W-jet (both daughters of W is withing jet cone). Only used for ttbar
          if TString(label).Contains("realW") and not getattr(treeIn,"Whadr_isW_def1"): continue
          if TString(label).Contains("fakeW") and     getattr(treeIn,"Whadr_isW_def1"): continue
          
          wtagger = getattr(treeIn,"Whadr_tau21")
          if options.usePuppiSD: wtagger = getattr(treeIn,"Whadr_puppi_tau2tau1")
          if options.useDDT:     wtagger = getattr(treeIn,"Whadr_puppi_tau2tau1")+ (0.063 * TMath.log( (pow( getattr(treeIn,"Whadr_puppi_softdrop"),2))/getattr(treeIn,"Whadr_puppi_pt") ))        
            
          discriminantCut = 0
          if   wtagger <= options.tau2tau1cutHP:					discriminantCut = 2 #HP
          elif wtagger > options.tau2tau1cutHP and wtagger <= options.tau2tau1cutLP:	discriminantCut = 1 #LP
          elif wtagger > options.tau2tau1cutLP:						discriminantCut = 0 #Extreme fail
      ...
      # ---------- DONE looping over tree entries ----------
      ...
      # Used to calculate extreme fail scalefactor by pure counting (too little events to use fit!)
